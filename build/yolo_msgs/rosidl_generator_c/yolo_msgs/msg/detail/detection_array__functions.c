  ...
  yolo_msgs__msg__DetectionArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    yolo_msgs__msg__DetectionArray * data =
      (yolo_msgs__msg__DetectionArray *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!yolo_msgs__msg__DetectionArray__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          yolo_msgs__msg__DetectionArray__fini(&data[i]);
        }
        free(data);
        return false;
      }
